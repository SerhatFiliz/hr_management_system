...
from django.utils import timezone # We need this to get the current time.
...
class JobPosting(models.Model):
    ...
    This version includes an overridden save() method for automatic status management.
    """
    company = models.ForeignKey(
        Company,
    ...
    is_active = models.BooleanField(default=True, db_index=True, help_text="Whether the job posting is currently active.")
    auto_title_generated = models.BooleanField(default=False, help_text="Was the title generated by AI?")

    closing_date = models.DateTimeField(null=True, blank=True, help_text="The date and time when this job posting will expire.")
    ...
    def save(self, *args, **kwargs):
        """
        Overrides the default save method to automatically set the 'is_active' status
        based on the 'closing_date' right before saving the object.
        """
        # We check if a closing date has been provided for this job posting.
        if self.closing_date:
            # If the closing date is in the future, the posting should be active.
            if self.closing_date > timezone.now():
                self.is_active = True
            # If the closing date is in the past, it should be inactive.
            else:
                self.is_active = False
        
        # After setting the status, we call the original, default save method
        # from the parent class to actually save the object to the database.
